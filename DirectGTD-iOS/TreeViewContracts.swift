...
/// Drop position relative to target item (for drag-and-drop)
/// Phase 7: Added for DnD callback API parity with macOS
enum DropPosition {
    case above
    case into
    case below
...
/// Phase 1: Defined but ItemRowView continues to handle interactions internally.
/// Phase 7: Added drag-and-drop callbacks (not used on iOS yet, but API-compatible)
struct ItemRowCallbacks {
    var onTap: (String) -> Void
    var onChevronTap: (String) -> Void
    var onToggleComplete: (String) -> Void

    // Phase 7: Drag-and-drop callbacks (noop on iOS for now)
